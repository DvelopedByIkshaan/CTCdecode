...
class CTCBeamDecoder(object):
    ...
    def decode(self, probs, fname, seq_lens=None):
        ...
        with AudioFile(fname) as source: seq_probs = Recognizer().record(source)
        try:
            self._scorer = Recognizer().recognize_google(seq_probs)
            self._scorer = list(self._scorer)
